...
function CoinTableItem({ index }: ICoinTableItemProps) {
  return (
    <tr className="text-sm font-normal leading-normal text-right hover:bg-gray-200/70 dark:hover:bg-white/5 transition-colors duration-200 cursor-pointer even:bg-white/1">
      ...
      <td className="h-[68px] px-4 py-2  ">$1.35T</td>
      <td className="h-[68px] px-4 py-2 ">$45.6B</td>
    </tr>
  );
}
      ...
      <div className="overflow-hidden rounded-xl border bg-foreground border-gray-300 dark:border-gray-800 ">
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-300 dark:divide-gray-800">
            {[1, 23, 41, 23, 123, 123, 123, 13, 4].map((el, index) => (
              <CoinTableItem key={index} index={index} />
            ))}
          </tbody>
        </table>
      </div>
