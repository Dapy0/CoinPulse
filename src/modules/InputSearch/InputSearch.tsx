      ...
      {isOpen && inputValue.length > 0 && (
        <div className="absolute top-full mt-2 left-0 w-full bg-foreground rounded-xl shadow-xl border border-gray-300 dark:border-gray-700 max-h-80 overflow-y-auto z-50">
          ...
          {!isLoading && searchResults?.length === 0 && inputValue.length > 2 && (
            <div className="p-4 text-center text-gray-500 text-sm wrap-break-word">No coins found for "{inputValue}"</div>
          )}


          {!isLoading && searchResults?.length > 1 &&
            searchResults.map((coin: { id: string, thumb: string, name: string, symbol: string, market_cap_rank: number, }) => (
              <div
                key={coin.id}
              ...
              >
                ...
                <div className="flex flex-col gap-1 min-w-0 ">
                  <span className="text-sm font-bold text-gray-900 dark:text-white leading-none truncate">{coin.name}</span>
